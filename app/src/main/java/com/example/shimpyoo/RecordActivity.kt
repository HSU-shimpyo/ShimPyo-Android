...
class RecordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 클릭 시 downloadRecording 메서드를 호출하여 녹음된 파일을 다운로드
        buttonDownload.setOnClickListener {
            downloadRecording()
        }

        // buttonUpload의 클릭 리스너 설정
        // 클릭 시 uploadRecording 메서드를 호출하여 녹음된 파일을 업로드
    ...
    private fun startRecording() {
        ...
        if (!state) {
            ...
            try {
                ...
                // 토스트 메시지 표시
                Toast.makeText(this@RecordActivity, "레코딩 시작", Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Recording", "Recording failed: ${e.message}")
                Toast.makeText(this@RecordActivity, "레코딩 오류 발생: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        } else {
    ...
    private fun startPlaying() {
        mediaPlayer = MediaPlayer().apply {
            try {
                ...
                prepare()
                start()
                Toast.makeText(this@RecordActivity, "재생 시작", Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Playback", "Playback failed: ${e.message}")
                Toast.makeText(this@RecordActivity, "재생 오류 발생: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun uploadRecording() {
        ...
        // Flask 서버의 로컬 IP 주소와 포트 입력 -> 맥 ip 주소로 찾아서 바꿔야함
        val url = "http://~~~:5001/analyze"

        // 요청 본문을 멀티파트 폼 데이터로 작성.
        ...
        // OkHttp 클라이언트를 사용하여 비동기적으로 요청을 보낸다.
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                e.printStackTrace()
                Log.e("Upload", "Failed to upload file: ${e.message}")
                runOnUiThread {
                    // 업로드 실패 시 토스트 메시지를 표시.
                    Toast.makeText(this@RecordActivity, "업로드 실패: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    Log.i("Upload", "Server response: $responseBody")
                    runOnUiThread {
                        // 업로드 성공 시 토스트 메시지를 표시.
                        Toast.makeText(this@RecordActivity, "업로드 성공", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    val errorMessage = response.message
                    Log.e("Upload", "Server responded with error: $errorMessage")
                    runOnUiThread {
                        // 서버 오류 시 토스트 메시지를 표시.
                        Toast.makeText(this@RecordActivity, "서버 오류: $errorMessage", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        })
